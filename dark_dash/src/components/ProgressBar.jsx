...
function ContextualExample() {
  ...
  return (
    <div>
      ...
      <ProgressBar variant="info" now={(totalGarbageTax / data.length) *100 } label={`${totalGarbageTax}%`} style={{ marginBottom: '25px' }} />
      <span>Water Tax</span>
      <ProgressBar variant="warning" now={(totalWaterTax / data.length) * 100} label={`${totalWaterTax}%`} style={{ marginBottom: '25px' }} />
      <span>Property Tax</span>
      <ProgressBar variant="danger" now={(totalPropertyTax / data.length) * 100} label={`${totalPropertyTax}%`} />
    </div>
  );
}
